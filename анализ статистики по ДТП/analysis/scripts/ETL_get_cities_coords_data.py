...
def clean_cities(df_cities: pd.DataFrame) -> pd.DataFrame:
    ...
    Возвращает очищенный pandas.DataFrame с пятью колонками:
...
def load_df_to_DB (df_1: pd.DataFrame, 
                   ...
                   df_3: pd.DataFrame, 
                    ...
                    df_name2: str,
                    df_name3: str,
                    backup_to_csv: bool = True,  # параметр для резервного копирования
                    csv_output_dir: str = "backup_csv"  # папка для CSV файлов
                   ) -> None:
    '''
    Функция загружает DataFrames в базу данных или csv файлы если база недоступна.
    ...
    if connection_ok == True:    
        ...
        logger.error(f'Ошибка при загрузке данных в базу')
...
if __name__ == '__main__':
    try:
        ...
        result_df = add_coordinates_to_cities(clean_cities)
        logger.info(f'Координаты добавлены для {result_df["lon"].notna().sum()} городов')

        load_df_to_DB(df_1=df_cities, df_2=clean_cities, df_3=result_df, 
                      df_name1 = 'wiki_cities', df_name2='clean_cities', df_name3 = 'cities_coords',
                    ) 
